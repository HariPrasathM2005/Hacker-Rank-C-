...
int birthdayCakeCandles(vector<long> candles) {
    long bigCandle = 0;
    int bigCandleCnt;
    for(unsigned int i = 0; i < candles.size(); i++)
    {
        if(bigCandle < candles[i])
        {
            bigCandle = candles[i];
            bigCandleCnt = 1;
        }
        else if(bigCandle == candles[i])
            bigCandleCnt++;
        cout << "Big Candle: " << bigCandle << " Big Candle Count: " << bigCandleCnt << "\n";
    }
    return bigCandleCnt;
}
...
int main()
{
    ...
    vector<long> candles(candles_count);
    for (int i = 0; i < candles_count; i++) {
        long candles_item = stol(candles_temp[i]);
